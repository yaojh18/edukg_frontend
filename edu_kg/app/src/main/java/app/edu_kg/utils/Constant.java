package app.edu_kg.utils;

public class Constant {
    public final static int MESSAGE_LIST_RESPONSE = 1001;
    public final static int LOGIN_RESPONSE = 1002;
    public final static int REGISTER_RESPONSE = 1003;
    public final static int MODIFY_RESPONSE = 1004;
    public final static int HOME_ENTITY_RESPONSE = 1005;
    public final static int LINK_INSTANCE_RESPONSE = 1006;
    public final static int INSTANCE_LIST_RESPONSE = 1007;
    public final static int LIST_RESPONSE = 1008;
}
